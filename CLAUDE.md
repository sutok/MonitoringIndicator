...
- **MT4/MT5のログファイルはサーバー時間で作成される**
  - システム時間（JST）とサーバー時間（GMT+2/GMT+3）は6〜7時間の時差がある
  - JSTで日付が変わってもサーバー時間では前日のため、ログファイルは前日の日付

- **推奨設定: ディレクトリのみ指定**
  ```yaml
  mt4:
    alert_log_path: "C:/Users/.../MQL4/Logs/"  # ディレクトリのみ
  ```
  - システムが自動的に最新の.logファイルを検出
  - タイムゾーン問題を完全回避
  - 新しいログファイル作成時も自動切り替え

- **動作:**
  - 起動時: ディレクトリ内の最新.logファイルを検出
  - 実行中: より新しい.logファイルが作成されたら自動切り替え
  - 日付変更時: 最新ファイルに自動的に切り替え
